...
class TopicService {
    ...
    ///
    static func loadFilteredItemsForTableView(topic: TopicElement) -> [[Item]] {
        var filteredItems = [[Item]]()
        let userMode = SettingsRepository.mode
        let regularItems = filterRegularItem(with: topic)
        let intermediateItems = filterIntermediateItem(with: topic)
        let advancedItems = filterAdvancedItem(with: topic)
        // add first the regular items if there is.
        if !regularItems.isEmpty {
            filteredItems.append(regularItems)
        }
        // next add intermediate if not empty +
    ...
    static func filterRegularItem(with topic: TopicElement) -> [Item] {
        return topic.items.filter { $0.mode == Mode.essential.jsonReferenceName }.sorted { $0.title < $1.title }
    }
    
    static func filterIntermediateItem(with topic: TopicElement) -> [Item] {
        topic.items.filter { $0.mode == Mode.intermediate.jsonReferenceName }.sorted { $0.title < $1.title }
    }
    
    static func filterAdvancedItem(with topic: TopicElement) -> [Item] {
        topic.items.filter { $0.mode == Mode.advanced.jsonReferenceName }.sorted { $0.title < $1.title }
    }
}
